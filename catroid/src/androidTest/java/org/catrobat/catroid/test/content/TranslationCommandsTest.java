...
public class TranslationCommandsTest extends AndroidTestCase {
	...
	protected void setUp() throws Exception {
		super.setUp();
		project = new Project(getContext(), "testProject" + System.currentTimeMillis(), false);
		ProjectManager.getInstance().setProject(project);
		ProjectManager.getInstance().saveProject(getContext());

		Scene scene = StorageHandler.getInstance().createDefaultScene(getString(TEST_SCENE, "en"), false, false, getContext());
	...
	public void testTranslationCommands() {
		...
		CommandFactory.translateScenesCommand(scenes, getContext()).execute();
		...
		sprites.add(sprite);
		CommandFactory.translateSpritesCommand(sprites, getContext()).execute();
		...
		CommandFactory.translateUserVariablesCommand(userVariables, userVariableFormulas, getContext()).execute();
		...
		CommandFactory.translateUserListsCommand(userLists, userListFormulas, getContext()).execute();
		...
		looks.add(look);
		CommandFactory.translateLooksCommand(looks, getContext()).execute();
		...
		CommandFactory.translateTranslatables(translatables, getContext()).execute();
